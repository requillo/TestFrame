...
<?php if(!empty($e_options)){
	...
	?>
		...
		<?php foreach ($e_options as $k => $val) { ?>
			...
			$class = 'form-control input-'.$val['id'];
			if($val['type'] == 3) {
				$class = 'flat input-'.$val['id'];
			}
			if($val['type'] == 2) {
				$step = 'step="any"';
			} else {
		  						...
		  						<tr>
		  							<?php foreach ($doptions as $key => $value) { ?>
		  								...
		  								if($value['data'][$x]['is_required'] == 1) {
											...
											$class = 'form-control linked-input-'.$value['id'];
											if($value['data'][$x]['type'] == 3) {
												$class = 'flat linked-input-'.$value['id'];
											}
...
<div class="flex">
		  		...
		  		<?php foreach ($value as $val) { ?>
		  			...
		  			<?php } ?>
		  					<?php 
									...
									$class = 'form-control imput-'.$val['id'];
									if($val['type'] == 3) {
										$class = 'flat input-'.$val['id'];
									}
									if($val['type'] == 2) {
									...
									<div class="input-group">
										<?php if($val['type'] == 4 || $val['type'] == 5) { ?>
											<select id="id-<?php echo $val['id'] ?>" name="data[data_checklist][<?php echo $val['id'] ?>]" class="<?php echo $class ?>">
												<?php echo $form->options($val['type_options']); ?>
											</select>
										<?php } else { ?>
											<input id="id-<?php echo $val['id'] ?>" type="<?php echo strtolower($option_types[$val['type']]) ?>" name="data[data_checklist][<?php echo $val['id'] ?>]" 
	...
	<script type="text/javascript">
		<?php echo $listname['custom_script']; ?>
	</script>
<?php } ?>	
