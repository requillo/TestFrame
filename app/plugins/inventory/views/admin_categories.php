...
<div class="container-fluid"> 
	<div class="panel panel-default">
		<div class="panel-body">
		  	<div class="row">
		  		<div class="col-lg-10">
		  			<?php $form->input('name',array('type'=>'text','label'=>__('Category name','inventory'))); ?>
		  		</div>
				...
				<thead>
          		...
          		<tbody>
          			...
          			<tr>
						...
						<td>
							<a href="#" class="edit-cat-name"><i class="fa fa-pencil" aria-hidden="true"></i> <?php echo $cat['name'] ?></a>
							<div class="info-cat hide">
								<input class="form-control input-name" type="text" value="<?php echo $cat['name'] ?>">
								<input class="input-id" type="hidden" value="<?php echo $cat['id'] ?>">
							</div>	
						</td>
						<td>
...
<script type="text/javascript">
	...
	$('.add-cat').on('click', function(e){
		...
		if(ee){
			...
			$.getJSON( Jsapi+'inventory/add-category/', fdata , function( data ) {
			console.log(data);
	...
	$('.edit-cat').on('click', function(e){
		...
		if(ee){
			...
			$.getJSON( Jsapi+'inventory/edit-category/', fdata , function( data ) {
			console.log(data);
				if(data.message == 'success'){
					location.reload();
				} else {
	...
	$('.delete-cat').on('click', function(e){
	    ...
	    buttons: {
	        confirm: {
	            ...
	            action: function(){
	            	$('#all-cat-wrapper').append(loader);
	            	$.getJSON( Jsapi+'inventory/delete-category/', {'data[id]': c} ,function( data ) {
	            		if(data.message == 'success'){
	            			location.reload();							
	            		} else {
	            			$('body').find('.loaderwrap').remove();
