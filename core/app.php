...
class App {
	...
	public function __construct() {
		...
		if($this->controller->isdbcon == 'No' && $app['install-the-app'] == true) {
			...
			die();
		} else if($this->controller->isdbcon == 'No' && $app['install-the-app'] == false) {
			echo 'Nothing here';
			if($app['app-debug'] == true) {
				echo '<br> There is no database connected to the application, if you want to install the application please set the <b>install-the-app</b> to true in configuration file';
			}
			return false;
		} else {
			// get lang file
			...
			if($admin_theme == '') {
				...
				if(file_exists($db_install)) {
					...
					echo json_encode(array('message' => 'Could not install application data', 'class' => 'error'));
				}
				die();	
		...
		if($app['show-website'] == false){
			...
			if(isset($_SESSION['loggedin']['id'])){
				// For the rest api ==================================================================================================
				// print_r($this->controller->user);
				if($this->controller->user['status'] == 0) {
				...
				if($this->url[0] == $app['api-url']){
					...
					if(isset($this->url[3])) {
						...
						$guard =  $this->controller->is_plugin($class);
				...
				if($this->url[0] == $app['cron-url']){
					...
					if(class_exists($this->url[1]) && $guard['allow'] == true ){
						$cron->job($this->url[1],$this->url[2]);
					}
					die();
				} // End Cron jobs
				// Do the logout stuff
			...
			// When no one is logged in
				...
				// This is the Rest Api ////////////////////////////////////////////////////////////////////////////////////////////////////////	
					...
					if(isset($this->url[2]) && $this->url[2] == $app['login-url']) {
						...
						$data = Rest::request();
						if(Rest::is_valid($data)){
							$this->controller->user = $this->controller->model->get_user_info($data['id']);
							$chpugin = $this->controller->is_plugin($this->url[2]);
							$class = str_replace('-', '_', $this->url[2]);
							$method = 'rest_api_'.str_replace('-', '_', $this->url[3]);
							$page = $class.'/'.$method;
							$allow = $this->controller->model->is_admin_page_allowed($this->controller->user['role_level'],$page);
							if($chpugin['message'] == 'plugin'){
								$plugin = new $class;
								if(method_exists($plugin,$method) && $allow == true){
									$plugin->$method();
								}
							} else if(class_exists($class)){
								$admin_enter = new $class();
								if(method_exists($admin_enter,$method) && $allow == true){
									$admin_enter->$method();
								}
							}
						}
		...
		// This area is for when website is enabled	
			...
			if(isset($_SESSION['loggedin']['id'])){
				...
				if($this->url[0] == $app['api-url']){
					if(isset($this->url[3])) {
						// Check if it's a plugin
						$this_class = str_replace('-', '_', $this->url[2]);
						$page = $this_class .'/'. str_replace('-', '_', $this->url[3]);
						$guard =  $this->controller->is_plugin($this_class);
						$allow = $this->controller->model->is_admin_page_allowed($this->controller->user['role_level'],$page);
						if($allow && $guard['allow'] == true) {
	...
	private  function token($arg) {
		if(isset($arg['token']) && isset($arg['action']) && isset($arg['request'])){
			$app = App::config('app');
			...
			if(class_exists($action[0]) && $guard['allow'] == true ) {
				...
				if(method_exists($class,$method)) {
					...
					// This is for the Hero role	
					} else if(isset($this->controller->user) &&  $this->controller->user['role_level'] > 99){
						echo 'Token: ' . $token_check;
						$class->$method();
						$viewvars = array();
						$viewvars['theme'] = $this->admin_theme;
						$viewvars['class'] =  $action[0];
	...
	private function admin(){
		...
		$viewvars['url'] = $this->url;
			...
			if($this->url[0] != 'index' ){
				...
				$allow = $this->controller->model->is_admin_page_allowed($this->controller->user['role_level'],$page);
				// Do not show these if website is disabled
				if($this->url[0] == 'pages') {
					$guard['allow'] = false;
				} else if($this->url[0] == 'web_settings') {
					$guard['allow'] = false;
				} else if($this->url[0] == 'forms') {
					$guard['allow'] = false;
				}
				// Check if the url is dashboard to do splash or not
	...
	private function front(){
		...
		$splash = $app['webthemes-path'] . $this->web_theme . '/splash.php';
		if(isset($this->url[0]) && $this->url[0] != rtrim(BACKEND_URL,'/')){
			...
			if($this->url[0] == 'index'){
				...
				if(!empty($page)){
					...
					$viewvars['page-title'] = __('Hello world');
					$viewvars['content'] = __('Your almost ready to show the world what you build');
				}
				...
				$chpugin = $this->controller->is_plugin($this->url[0]);
				if($chpugin['message'] == 'plugin'){
					$class = str_replace('-', '_', $this->url[0]);
					$plugin = new $class;
					if(!isset($this->url[1])) {
						$method = 'website_index';
					} else {
						$method = 'website_'.str_replace('-', '_', $this->url[1]);
					}
					// Set variable for attributes
					$strvars = '';
					if(isset($this->url[2])) {
					...
					// Don not call widgets
					$method = str_replace('widget_', '', $method);
					if(method_exists($plugin,$method)){
						$plugin->$method();
						$urlvars = array('urlvars' => $strvars );
						$plugin->var = array_merge($plugin->var, $urlvars);
						if(isset($plugin->title) && $plugin->title != '') {
							$viewvars['page-title'] = $plugin->title;
						}
					...
					// echo $page['forms'];
				if($page['forms'] != '') {
						$formhtml = $this->controller->get_page_forms($page['forms']);
					} else {
						$formhtml = '';
					}
				...
				$viewvars['page-title'] = $page['title'];
				$viewvars['content'] = $page['content'];
				$viewvars['formhtml'] = $formhtml;
				} else {
					$viewvars['page-title'] = 'Thank you for contacting us';
					$viewvars['content'] = 'Thank you, message has been sent';
				} 
				$this->controller->view->theme($theme,$viewvars);
	...
	private function get_page($slug){
		$sql = "SELECT * FROM ".PRE."pages WHERE slug = :slug AND status = 1";
